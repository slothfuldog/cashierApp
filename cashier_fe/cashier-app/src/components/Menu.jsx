...
import { addItems, deleteItems } from "../action/orderAction";
...
const Menu = (props) => {
  ...
  const setString = () => {
    if (props.data.description.length > 32) {
      setStr(`${props.data.description.substring(0, 34)}...`);
    } else {
      setStr(props.data.description);
    }
  };  
  const setStringName = () => {
    if (props.data.name.length > 24) {
      setStrName(`${props.data.name.substring(0, 25)}...`);
    } else {
